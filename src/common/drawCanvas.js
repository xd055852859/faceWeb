const DrawCanvas = {
  maxNum(arr) {
    ...
    console.log(canvasName,canvasDom);
    if (canvasDom) {
      console.log(canvasName);
      const context = canvasDom.getContext("2d");
      let backgroundNumArr = [
        13, 22, 24, 26, 17, 15, 13, 19, 17, 50, 58, 64, 62, 53, 30, 39, 41, 43,
        34, 32, 30, 36, 34,
      ];
      ...
      if (drawBgContext) {
        ...
        if (background) {
          ...
          contextBackground.strokeStyle = "rgba(255,255,255,0.6)";
          ...
          backgroundNumArr.forEach((item, index) => {
            ...
            contextBackground.beginPath();
            contextBackground.arc(
              backgroundArr[item].x,
              backgroundArr[item].y,
              0.9,
              0,
              2 * Math.PI
            );
            if (index < backgroundNumArr.length - 1) {
              this.drawRectLine(
                backgroundArr[item].x,
                backgroundArr[item].y,
                backgroundArr[backgroundNumArr[index + 1]].x,
                backgroundArr[backgroundNumArr[index + 1]].y,
                contextBackground
              );
            }
            contextBackground.strokeStyle = "rgba(255,255,255,0.3)";
            contextBackground.stroke();
            contextBackground.closePath();
          });
        }
